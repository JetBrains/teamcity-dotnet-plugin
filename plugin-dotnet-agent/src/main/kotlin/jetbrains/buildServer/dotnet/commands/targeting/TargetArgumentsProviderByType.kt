...
class TargetArgumentsProviderByType(
    ...
    override fun getTargetArguments(targets: Sequence<CommandTarget>) = when {
        isEnabled -> splitByTargetType(targets)
        else -> splitByDefault(targets)
    }

    private val isEnabled get() =
        _parametersService.tryGetParameter(ParameterType.Runner, PARAM_SINGLE_SESSION)?.trim()?.toBoolean() ?: false
