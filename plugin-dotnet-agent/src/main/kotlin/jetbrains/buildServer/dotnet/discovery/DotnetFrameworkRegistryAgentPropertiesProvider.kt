                    ...
                    return@map curFrameworks
                            ...
                            .map {
                                ...
                                sequence {
                                    ...
                                    yield(AgentProperty(ToolInstanceType.DotNetFramework, "$CONFIG_PREFIX_DOTNET_FRAMEWORK${majorVersion}_${it.platform.id}", it.version.toString()))
                                    yield(AgentProperty(ToolInstanceType.DotNetFramework, "$CONFIG_PREFIX_DOTNET_FRAMEWORK${majorVersion}_${it.platform.id}_Path", it.path.path))
                                    yield(AgentProperty(ToolInstanceType.DotNetFramework, "$CONFIG_PREFIX_DOTNET_FRAMEWORK${it.version}_${it.platform.id}_Path", it.path.path))
                                }
                            }
                            .flatMap { it }
                }
