    ...
    override fun create(destinationStream: OutputStream, outputFile: Path, cachesHomeDirectory: Path?, debug: Boolean) =
        _xmlWriter.write(
                E("DupFinderOptions",
                        ...
                        E("DiscardFieldsName", _parametersService.tryGetParameter(ParameterType.Runner, SETTINGS_DISCARD_FIELDS_NAME)?.let { it.toBoolean().toString() }),
                        E("DiscardLiterals", _parametersService.tryGetParameter(ParameterType.Runner, SETTINGS_DISCARD_LITERALS)?.let { it.toBoolean().toString() }),
                        E("DiscardLocalVariablesName", _parametersService.tryGetParameter(ParameterType.Runner, SETTINGS_DISCARD_LOCAL_VARIABLES_NAME)?.let { it.toBoolean().toString() }),
                        E("DiscardTypes", _parametersService.tryGetParameter(ParameterType.Runner, SETTINGS_DISCARD_TYPES)?.let { it.toBoolean().toString() }),
                        E("NormalizeTypes", _parametersService.tryGetParameter(ParameterType.Runner, SETTINGS_NORMALIZE_TYPES)?.let { it.toBoolean().toString() }),
