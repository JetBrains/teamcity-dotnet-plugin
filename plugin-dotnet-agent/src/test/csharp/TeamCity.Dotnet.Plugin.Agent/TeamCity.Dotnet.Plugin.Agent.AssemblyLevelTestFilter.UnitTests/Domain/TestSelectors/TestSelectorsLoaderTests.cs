...
public class TestSelectorsLoaderTests
{
    ...
    public TestSelectorsLoaderTests()
    {
        ...
        _loader = new TestSelectorsLoader(_selectorParserMock.Object, _fileSystemMock.Object, loggerMock.Object);
    }
    ...
    [Fact]
    public async Task LoadTestSelectorsFromAsync_Should_Skip_Control_Lines()
    {
        _fileSystemMock.Setup(fs => fs.FileExists(It.IsAny<string>())).Returns(true);
        _fileSystemMock.Setup(fs => fs.GetFileInfo(It.IsAny<string>())).Returns((new FileInfo("path/to/file"), null));
        _fileSystemMock.Setup(fs => fs.ReadLinesAsync(It.IsAny<string>())).Returns(ToAsyncEnumerable(new List<(string, int)> { ("#this is a comment", 1) }));
        var result = await _loader.LoadTestSelectorsFromAsync("path/to/file");
        Assert.Empty(result);
    }
    ...
    public async Task LoadTestSelectorsFromAsync_Should_Skip_Empty_Lines()
    {
        _fileSystemMock.Setup(fs => fs.FileExists(It.IsAny<string>())).Returns(true);
        _fileSystemMock.Setup(fs => fs.GetFileInfo(It.IsAny<string>())).Returns((new FileInfo("path/to/file"), null));
        _fileSystemMock.Setup(fs => fs.ReadLinesAsync(It.IsAny<string>())).Returns(ToAsyncEnumerable(new List<(string, int)> { ("", 1) }));
        var result = await _loader.LoadTestSelectorsFromAsync("path/to/file");
        Assert.Empty(result);
    }
    ...
    public async Task LoadTestSelectorsFromAsync_Should_Return_Correct_Selector_When_Parser_Returns_True()
    {
        ...
        _fileSystemMock.Setup(fs => fs.FileExists(It.IsAny<string>())).Returns(true);
        _fileSystemMock.Setup(fs => fs.GetFileInfo(It.IsAny<string>())).Returns((new FileInfo("path/to/file"), null));
        _fileSystemMock.Setup(fs => fs.ReadLinesAsync(It.IsAny<string>())).Returns(ToAsyncEnumerable(new List<(string, int)> { ("valid_selector", 1) }));
