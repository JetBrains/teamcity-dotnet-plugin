...
class CommandLinePresentationServiceTest {
    ...
    fun testExecutableFilePresentation(): Array<Array<out Any?>> {
        return arrayOf(
                arrayOf(File("Dotnet.exe"), listOf(StdOutText("Dotnet.exe", Color.Header))),
                arrayOf(File("Dir", "Dotnet.exe"), listOf(StdOutText("Dir" + File.separator, Color.Minor), StdOutText("Dotnet.exe", Color.Header))))
    }

    @Test(dataProvider = "testExecutableFilePresentation")
    fun shouldBuildExecutableFilePresentation(executableFile: File, expectedOutput: List<StdOutText>) {
        // Given
        var presentation = createInstance()

        // When
        val actualOutput = presentation.buildExecutableFilePresentation(executableFile)
