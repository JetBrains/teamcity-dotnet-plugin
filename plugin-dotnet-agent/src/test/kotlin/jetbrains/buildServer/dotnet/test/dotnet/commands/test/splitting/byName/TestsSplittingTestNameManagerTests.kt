...
import jetbrains.buildServer.dotnet.commands.test.splitting.byTestName.*
import org.testng.Assert
import org.testng.annotations.BeforeMethod
import org.testng.annotations.Test

class TestsSplittingTestNameManagerTests {
    @MockK
    ...
    fun `should start session for no test classes`() {
        // arrange
        val filterType = TestsSplittingFilterType.Includes
    ...
    fun `should save valid test name with excludes filter session`() {
        // arrange
        val filterType = TestsSplittingFilterType.Excludes
        val testClasses = sequenceOf("Namespace.TestClass0", "Namespace.TestClass1")
        _settingsMock.also { it ->
            every { it.filterType } answers { filterType }
            every { it.testClasses } answers { testClasses }
    ...
    private fun create() = TestsSplittingTestNamesManager(_settingsMock, _testListFactoryMock, _listTestsOutputValueProcessor)
}