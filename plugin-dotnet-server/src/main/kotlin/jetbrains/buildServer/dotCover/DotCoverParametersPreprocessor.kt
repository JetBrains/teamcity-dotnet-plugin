...
class DotCoverParametersPreprocessor : ParametersPreprocessor {
    override fun fixRunBuildParameters(
        ...
        val isDotCoverHomeParameterPresent = runParameters[CoverageConstants.PARAM_DOTCOVER_HOME] != null
        if (dotCoverDisabled(runParameters) && isDotCoverHomeParameterPresent) {
            // it's needed to remove reference on tool from runner parameters.
            // otherwise if dotcover tool doesn't exist on agent
            // will report warning message in build log. See TW-60495
